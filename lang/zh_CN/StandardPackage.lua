...
local t = {
	...
	["collateral-combat"] = "%src 使用了【腹黑的骗术】，目标是 %dest，请打出一张战斗牌以响应", 
...
使用目标：除你以外，装备区里有武器牌的一名角色A。（你需要在此阶段选择另一个A使用战斗牌能攻击到的角色B）。\
作用效果：A需对B使用一张战斗牌，否则A必须将其装备的武器牌交给你", 
	[":indulgence"] = "出牌时机：出牌阶段。\
使用目标：除你以外，任意一名角色。\
作用效果：将【永夜异变】横置于目标角色判定区里，目标角色回合判定阶段，进行判定；若判定结果不为红桃，则跳过目标角色的出牌结算，将【永夜异变】弃置", 
